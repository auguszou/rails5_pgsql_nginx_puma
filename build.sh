...
# get latest version
latest_version=$(docker images | grep zouqilin\/rails5_web | grep -v "latest" | grep -v "previous" | grep -v "current" | awk '{print $2}' | sort -r --version-sort | head -1)

[[ "$latest_version" == "" ]] && latest_version="0.0.0"
echo "current version is: $latest_version"
previous_version=$latest_version

[[ "$v" == "major" ]] && latest_version=$(increment_version "$latest_version" 1)
[[ "$v" == "minor" ]] && latest_version=$(increment_version "$latest_version" 2)
[[ "$v" == "revision" ]] && latest_version=$(increment_version "$latest_version" 3)
echo "build version: $latest_version"

[[ "$previous_version" != "0.0.0" ]] && docker tag zouqilin/rails5_web:${previous_version} zouqilin/rails5_web:previous
docker build -t zouqilin/rails5_web:${latest_version} .
docker tag zouqilin/rails5_web:${latest_version} zouqilin/rails5_web:current
